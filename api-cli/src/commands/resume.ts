    ...
    .action(async () => {
        ...
        if (!config.get("remote_station")) {
            try {
                ...
                spinner.stop();
                console.log(chalk.magentaBright(`✓ Successfully resumed the owvision demon`));
            } catch (err) {
                spinner.stop();
                console.log(chalk.yellow(`⚠️  Failed to resume the owvision demon`));
            }
        } else {
            console.log(chalk.yellow(`⚠️  Cannot resume a remote owvision demon. You have to resume it on the host machine.`));
        }
    });
