...
const summary_command = new Command("summary")
    .alias("sm")
    .description(
        "Get a summary of all sensors (max value measured, min value measured, etc)"
    )
    ...
    .addOption(
        new Option(
            "-h, --hour [hour in dd.mm.yyyy:HH]",
            "Get the summary of the passed hour, if no hour is passed the current one is taken"
        )
    )
    .addOption(
        new Option(
            "-d, --day [date in dd.mm.yyyy]",
            "Get the summary of the passed date, if no date is passed the current one is taken"
        )
    )
    .addOption(
        new Option(
            "-w, --week [week in yyyy:week]",
            "Get the summary of the passed week, if no week is passed the current one is taken"
        )
    )
    .addOption(
        new Option(
            "-m, --month [month in mm.yyyy]",
            "Get the summary of the passed month, if no month is passed the current one is taken"
        )
    )
    .addOption(
        new Option(
            "-y, --year [year in yyyy]",
            "Get the summary of the passed year, if no year is passed the current one is taken"
        )
    )
    .addOption(new Option("--alltime", "get the alltime summary (default)"))
    .action(async (station_name, options) => {
        ...
        let url = `${config.get(
            "api_url"
        )}/weather-stations/${station_name}/summaries`;
        if (typeof options[type] === "string") {
            try {
                ...
                console.log(
                    `${chalk.redBright(
                        `✘ Invalid option argument for option '--${type}' has been passed`
                    )}`
                );
                return;
            }
        ...
        try {
            ...
            if (!response.data.success) {
                if (response.data.error.code === "E_VALIDATION_ERROR") {
                    ...
                    return console.log(
                        `${chalk.redBright(
                            `✘ Invalid ${type} format, valid example: ${chalk.italic(
                                example
                            )}`
                        )}`
                    );
            ...
            for (const record of summary.records) {
                if (
                    ...
                    continue;
                if (
                    sensor_names?.length > 0 &&
                    !sensor_names.includes(record.sensor_slug)
                )
                    continue;
                let record_string = "";
                switch (record.summary_type) {
                    case "max":
                        record_string = `\t❯ ${chalk.magentaBright(
                            "max:"
                        )} ${safe_value_with_unit(
                            record.max_value,
                            record.unit
                        )} at ${
                            record.max_time
                    ...
                    case "latest":
                        record_string = `\t❯ ${chalk.magentaBright(
                            ...
                            record.time
                                ? chalk.italic(
                                      DateTime.fromISO(record.time).toFormat(
                                          "dd.MM.yyyy (HH:mm)"
                                      )
                                  )
                                : "-"
                    ...
                    case "sum":
                        record_string = `\t❯ ${chalk.magentaBright(
                            "sum:"
                        )} ${safe_value_with_unit(record.value, record.unit)}`;
                        break;
                    case "custom":
                        record_string = `\t❯ ${safe_value_with_unit(
                            record.value,
                            record.unit
                        )}`;
                        break;
                ...
                console.log(
                    `${chalk.blueBright(record.sensor_name)} ${chalk.bold(
                        `(${record.sensor_slug})`
                    )}`
                );
