        ...
        try {
            ...
            if (!response.data.success) { 
                if(response.data.error.code === "E_VALIDATION_ERROR"){
                    let example = "";
                    switch(type){
                        case "hour": example = "08.12.2022:14"; break;
                        case "day": example = "03.07.2024"; break;
                        case "week": example = "2024:32"; break;
            ...
            for (const record of summary.records) {
                ...
                switch (record.summary_type) {
                    case "max":
                        record_string = `\t❯ ${chalk.magentaBright('max:')} ${safe_value_with_unit(record.max_value, record.unit)} at ${record.max_time ? chalk.italic(DateTime.fromISO(record.max_time).toFormat("dd.MM.yyyy (HH:mm)")) : '-'}`
                        break;
                    case "min":
                        record_string = `\t❯ ${chalk.magentaBright('min:')} ${safe_value_with_unit(record.min_value, record.unit)} at ${record.min_time ? chalk.italic(DateTime.fromISO(record.min_time).toFormat("dd.MM.yyyy (HH:mm)")) : '-'}`
                        break;
                    case "min-max":
                        record_string = `\t❯ ${chalk.magentaBright('min:')} ${safe_value_with_unit(record.min_value, record.unit)} at ${record.min_time ? chalk.italic(DateTime.fromISO(record.min_time).toFormat("dd.MM.yyyy (HH:mm)")) : '-'}\n`
                        record_string += `\t❯ ${chalk.magentaBright('max:')} ${safe_value_with_unit(record.max_value, record.unit)} at ${record.max_time ? chalk.italic(DateTime.fromISO(record.max_time).toFormat("dd.MM.yyyy (HH:mm)")) : '-'}`
                    ...
                    case "min-avg":
                        record_string = `\t❯ ${chalk.magentaBright('min:')} ${safe_value_with_unit(record.min_value, record.unit)} at ${record.min_time ? chalk.italic(DateTime.fromISO(record.min_time).toFormat("dd.MM.yyyy (HH:mm)")) : '-'} \n`
                        record_string += `\t❯ ${chalk.magentaBright('avg:')} ${safe_value_with_unit(record.avg_value, record.unit)}`
                        break;
                    case "max-avg":
                        record_string = `\t❯ ${chalk.magentaBright('max:')} ${safe_value_with_unit(record.max_value, record.unit)} at ${record.max_time ? chalk.italic(DateTime.fromISO(record.max_time).toFormat("dd.MM.yyyy (HH:mm)")) : '-'}\n`
                        record_string += `\t❯ ${chalk.magentaBright('avg:')} ${safe_value_with_unit(record.avg_value, record.unit)}`
                    ...
                    case "avg":
                        ...
                        break;
                    case "latest":
                        record_string = `\t❯ ${chalk.magentaBright('latest:')} ${safe_value_with_unit(record.value, record.unit)} at ${record.time ? chalk.italic(DateTime.fromISO(record.time).toFormat("dd.MM.yyyy (HH:mm)")) : '-'}`
                        break;
                    case "oldest":
                        record_string = `\t❯ ${chalk.magentaBright('oldest:')} ${safe_value_with_unit(record.value, record.unit)} at ${record.time ? chalk.italic(DateTime.fromISO(record.time).toFormat("dd.MM.yyyy (HH:mm)")) : '-'}`
                        break;
                    case "sum":
                        record_string = `\t❯ ${chalk.magentaBright('sum:')} ${safe_value_with_unit(record.value, record.unit)}`
                        break;
                    case "custom":
                        record_string = `\t❯ ${safe_value_with_unit(record.value, record.unit)}`
                        break;
                }
                console.log(`${chalk.blueBright(record.sensor_name)} ${chalk.bold(`(${record.sensor_slug})`)}`)
