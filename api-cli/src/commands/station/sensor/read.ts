    ...
    .action(async (station_name, options) => {
        ...
        try {
            ...
            if (!sensors_response.data.success) {
                spinner.stop();
                return error_handling(sensors_response, {
            ...
            for (const sensor of sensors) {
                if (
                    ...
                    !sensor_names.includes(sensor.slug)
                )
                    continue;
                try {
                    ...
                    if (!sensor_data_response.data.success) {
                        spinner.stop();
                        return error_handling(sensor_data_response, {
                            station_slug: station_name,
            ...
            for (const sensor of sensors) {
                ...
                const seconds_ago =
                    record.created_at !== null
                        ...
                        : null;
                console.log(
                    `${chalk.blueBright(sensor.name)} ${chalk.bold(
                        `(${sensor.slug})`
