    ...
    .action(async (station_name) => {
        ...
        try {
            const response = await axios({
                url: `${config.get("api_url")}/weather-stations/${station_name}/down`,
                method: "post",
                headers: {
            ...
            console.log(`${chalk.magentaBright(`✓ Station ${chalk.bold(station_name)} is inactive`)}`)
        } catch (err) {
            spinner.stop()
