    ...
    .action(async () => {
        ...
        try {
            ...
            for (const station of stations) {
                const active = station.target_state === "active";
                const selected =
                    config.get("selected_station") === station.slug;
                console.log(
                    `${
                        station.remote_recorder
                            ? `${chalk.bgMagenta.white(`remote`)} `
                            : ""
                    }${chalk.blueBright(
                        selected ? chalk.underline(station.name) : station.name
                    )} ${chalk.bold(`(${station.slug})`)} ❯ [${
                        active
                            ? chalk.green(station.target_state)
                            : chalk.redBright(station.target_state)
                    }]`
                );
            }
            if (stations.length === 0) {
                console.log(
                    chalk.grey(
                        chalk.italic(
                            `There are no weather stations configured currently!`
                        )
                    )
                );
            }
