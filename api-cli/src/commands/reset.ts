...
const clear_command = new Command("reset")
        ...
        const response = await prompts([
            {
                ...
                message: chalk.redBright(
                    "Do you really want to reset owvision? All your data is lost!"
                ),
            },
        ]);
        ...
        if (response.confirm) {
            ...
            if (!config.get("remote_station")) {
                try {
                    ...
                    console.log(
                        chalk.magentaBright(
                            `✓ Successfully stopped the owvision demon and removed all your data.`
                        )
                    );
                } catch (err) {
                    spinner.stop();
                    console.log(
                        chalk.yellow(`⚠️  Failed to stop the owvision demon`)
                    );
                }
            } else {
                console.log(
                    chalk.yellow(
                        `⚠️  Cannot stop a remote owvision demon. You have to stop it on the host machine.`
                    )
                );
            }
        }
