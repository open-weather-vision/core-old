...
const transformerCreators = {
  ...
  pressure(targetUnit: 'hPa' | 'inHg' | 'mmHg' | 'mb') {
    switch (targetUnit) {
      ...
      case 'mb':
      case 'hPa':
        return (val: number | null) => (val === null ? null : val * 33.86389)
      case 'mmHg':
        return (val: number | null) => (val === null ? null : val * 25.4000003000246)
    }
  },
...
export function createUnitTransformers(unitSettings: UnitSettings): UnitTransformers {
  return {
    ...
    solarRadiation: transformerCreators.solarRadiation(unitSettings.solarRadiation),
  }
}
